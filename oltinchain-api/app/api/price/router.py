...
async def get_price_history(
    ...
    for i in range(limit - 1, -1, -1):
        ...
        open_price = base_price * (1 + Decimal(str((hash(str(candle_time) + "o") % 100 - 50) / 10000)))
        close_price = base_price * (1 + Decimal(str((hash(str(candle_time) + "c") % 100 - 50) / 10000)))
        
        high_price = max(open_price, close_price) * (1 + volatility)
        low_price = min(open_price, close_price) * (1 - volatility)
        
        candles.append(PriceHistoryItem(
