...
_db_url = os.getenv("DATABASE_URL", "postgresql://postgres:SOJ0Z74xsCIbd4mjD3de4z6LAgjWsGvs@postgres:5432/oltinchain")
...
async def get_db_stats():
    ...
    try:
        ...
        market_price = Decimal(str(totals["market_price"])) if totals and totals["market_price"] else Decimal("61")
        target_price = Decimal("61")
        deviation = ((market_price - target_price) / target_price) * 100 if target_price > 0 else Decimal("0")
        ...
        return {
            ...
            "total_trades": 0, "imbalance": 0, "market_price": 61, "target_price": 61,
            "deviation": 0, "trades": [],
        }
...
async def get_orderbook():
    ...
    try:
        ...
        return OrderBook(
            asks=ask_entries, bids=bid_entries,
            spread=str(spread), spread_percent=f"{float(spread_pct):.2f}",
            timestamp=datetime.utcnow()
        )
    except Exception as e:
        print(f"OrderBook Error: {e}")
        return OrderBook(asks=[], bids=[], spread="1.00", spread_percent="1.64", timestamp=datetime.utcnow())
